...
- `middleware/db_session.py` — `DbSessionMiddleware` calls `session.remove()` in a
  `finally`, which is what makes the scoped session per-request. **It is not yet
  registered in `settings.MIDDLEWARE`** — without it the scoped session leaks
  state across requests. Register it (and `middleware.Middleware.SimpleMiddleware`,
  a request/response logger writing to `Logs.txt`) when wiring up views.
